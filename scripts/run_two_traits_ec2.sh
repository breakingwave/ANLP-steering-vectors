#!/usr/bin/env bash
# Run evil + sycophancy traits on EC2 A10G, upload to S3, then terminate the instance.
#
# Usage:
#   S3_BUCKET=my-bucket OPENAI_API_KEY=sk-... ./scripts/run_two_traits_ec2.sh
#
# Optional env vars:
...
#   TARGET_MODEL     — HuggingFace model (default: meta-llama/Llama-3.1-8B-Instruct)
#   GENERATOR_MODEL  — artifact generator (default: gpt-5.4-nano)
#   JUDGE_MODEL      — judge model (default: gpt-5.4-nano)
#   SKIP_TERMINATE   — set to "1" to skip instance termination (for debugging)
set -euo pipefail
...
SKIP_TERMINATE="${SKIP_TERMINATE:-0}"

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"

echo "=== EC2 two-trait run ==="
echo "Traits: evil, sycophancy"
...
if [ "$SKIP_TERMINATE" = "1" ]; then
  echo "SKIP_TERMINATE=1 — skipping instance termination"
  exit 0
fi

echo "Terminating this EC2 instance in 10 seconds... (Ctrl-C to abort)"
sleep 10
...
if [ -z "$INSTANCE_ID" ]; then
  echo "WARNING: Could not detect EC2 instance ID. Not terminating."
  exit 0
fi
